...
public class CalendarUseCaseTests
{
    ...
    [Fact]
    public async Task GetAllEvents_ReturnsCorrectEvents()
    {
        ...
        A.CallTo(() => _eventRepository.GetPhysicalActivity(patientId)).Returns(Task.FromResult((IEnumerable<PhysicalActivityEvent>)physicalActivityEvents));

        var foodEvents = new List<FoodEvent>
        {
            new FoodEvent { DateEvent = new DateTime(2024, 6, 1, 12, 0, 0), IngredientName = "Apple" }
        };
        A.CallTo(() => _eventRepository.GetFoods(patientId)).Returns(Task.FromResult((IEnumerable<FoodEvent>)foodEvents));

        var examEvents = new List<ExamEvent>
        {
            new ExamEvent { DateEvent = new DateTime(2024, 6, 1, 9, 0, 0), Title = "Blood Test" }
        };
        A.CallTo(() => _eventRepository.GetExams(patientId)).Returns(Task.FromResult((IEnumerable<ExamEvent>)examEvents));
        ...
        A.CallTo(() => _eventRepository.GetGlycemia(patientId)).Returns(Task.FromResult((IEnumerable<GlucoseEvent>)glucoseEvents));
        ...
        A.CallTo(() => _eventRepository.GetInsulin(patientId)).Returns(Task.FromResult((IEnumerable<InsulinEvent>)insulinEvents));

        var healthEvents = new List<HealthEvent>
        {
            new HealthEvent { DateEvent = new DateTime(2024, 6, 1, 10, 0, 0), Title = "Check-up" }
        };
        A.CallTo(() => _eventRepository.GetHealth(patientId)).Returns(Task.FromResult((IEnumerable<HealthEvent>)healthEvents));
        ...
        A.CallTo(() => _eventRepository.GetMedicalVisit(patientId)).Returns(Task.FromResult((IEnumerable<MedicalVisitEvent>)medicalVisitEvents));
